...
function searchData(){
    let inputData = document.forms['searchForm']['search'].value;
    if(inputData.length !== 0 && inputData.length >= 3) {
        ...
        xhttp.open("GET", "/search/" + inputData, true);
        xhttp.send();
...
function removeCardItem(key){
    ...
    xhttp.open("GET", "/secretProtocol/card/"+key+"/delete", true);
    xhttp.send();
}